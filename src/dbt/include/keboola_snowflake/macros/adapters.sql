...
{% macro keboola_snowflake__get_columns_in_relation(relation) -%}
  {%- set sql -%}
...
{% macro keboola_snowflake__show_object_metadata(relation) %}
  {%- set sql -%}
...
{% macro keboola_snowflake__list_schemas(database) -%}
...
{% macro keboola_snowflake__check_schema_exists(information_schema, schema) -%}
...
{% macro keboola_snowflake__alter_column_type(relation, column_name, new_column_type) -%}
...
{% macro keboola_snowflake__alter_relation_comment(relation, relation_comment) -%}
...
{% macro keboola_snowflake__alter_column_comment(relation, column_dict) -%}
...
{% macro keboola_snowflake__set_query_tag() -%}
...
{% macro keboola_snowflake__unset_query_tag(original_query_tag) -%}
  {% set new_query_tag = config.get('query_tag') %}
  {% if new_query_tag %}
...
{% macro keboola_snowflake__get_column_data_type_for_alter(relation, column) %}
  {#
...
{% macro keboola_snowflake__alter_relation_add_remove_columns(relation, add_columns, remove_columns) %}
    ...
    {% set sql -%}
       alter {{ relation.get_ddl_prefix_for_alter() }} {{ relation_type }} {{ relation.render() }} add column
          {% for column in add_columns %}
            {{ adapter.quote(column.name) }} {{ keboola_snowflake__get_column_data_type_for_alter(relation, column) }}{{ ',' if not loop.last }}
          {% endfor %}
    {%- endset -%}
...
{% macro keboola_snowflake__is_catalog_linked_database(relation=none, catalog_relation=none) -%}
...
{% macro keboola_snowflake_dml_explicit_transaction(dml) %}
  {#
...
{% macro keboola_snowflake__truncate_relation(relation) -%}
  {% set truncate_dml %}
  ...
  {% call statement('truncate_relation') -%}
    {% if keboola_snowflake__is_catalog_linked_database(relation=config.model) %}
        {{ truncate_dml }}
    {% else %}
      {{ keboola_snowflake_dml_explicit_transaction(truncate_dml) }}
    {% endif %}
  {%- endcall %}
{% endmacro %}

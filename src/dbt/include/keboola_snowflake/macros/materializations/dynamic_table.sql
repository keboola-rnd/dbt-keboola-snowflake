{% materialization dynamic_table, adapter='keboola_snowflake' %}
...
{% macro dynamic_table_get_build_sql(existing_relation, target_relation) %}
    ...
    {% else %}
        ...
        {% set configuration_changes = keboola_snowflake__get_dynamic_table_configuration_changes(existing_relation, config) %}
        ...
        {% elif on_configuration_change == 'apply' %}
            {% set build_sql = keboola_snowflake__get_alter_dynamic_table_as_sql(existing_relation, configuration_changes, target_relation, sql) %}
...
{% macro keboola_snowflake__get_dynamic_table_configuration_changes(existing_relation, new_config) -%}
    {% set _existing_dynamic_table = keboola_snowflake__describe_dynamic_table(existing_relation) %}

{% macro keboola_snowflake__scalar_function_create_replace_signature_sql(target_relation) %}
...
{% macro keboola_snowflake__scalar_function_body_sql() %}
    $$
{{ model.compiled_code }}
    $$
{% endmacro %}

{% macro keboola_snowflake__formatted_scalar_function_args_sql() %}
    {% set args = [] %}
...
{% macro keboola_snowflake__scalar_function_volatility_sql() %}
...
{% macro keboola_snowflake__scalar_function_create_replace_signature_python(target_relation) %}
    CREATE OR REPLACE FUNCTION {{ target_relation.render() }} ({{ formatted_scalar_function_args_sql()}})
    RETURNS {{ model.returns.data_type }}
    LANGUAGE PYTHON
...
{% macro keboola_snowflake__scalar_function_python(target_relation) %}
    {{ keboola_snowflake__scalar_function_create_replace_signature_python(target_relation) }}

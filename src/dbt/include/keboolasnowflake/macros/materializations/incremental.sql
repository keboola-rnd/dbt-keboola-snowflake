...
{% materialization incremental, adapter='keboola_snowflake', supported_languages=['sql', 'python'] -%}
  ...
  {%- set is_catalog_linked_db = keboola_snowflake__is_catalog_linked_database(relation=none, catalog_relation=catalog_relation) -%}
  ...
  {% else %}
    ...
    {% elif tmp_relation_type == 'view' %}
        {%- call statement('create_tmp_relation') -%}
          {{ keboola_snowflake__create_view_as_with_temp_flag(tmp_relation, compiled_code, True) }}
        {%- endcall -%}
    {% else %}
        {%- call statement('create_tmp_relation', language=language) -%}
...
{% macro keboola_snowflake__get_incremental_default_sql(arg_dict) %}

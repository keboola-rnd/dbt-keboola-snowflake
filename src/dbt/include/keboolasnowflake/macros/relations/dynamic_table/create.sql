{% macro keboola_snowflake__get_create_dynamic_table_as_sql(relation, sql) -%}

    {%- set catalog_relation = adapter.build_catalog_relation(config.model) -%}
    {%- set dynamic_table = relation.from_config(config.model) -%}

    {%- if catalog_relation.catalog_type == 'INFO_SCHEMA' -%}
        {{ keboola_snowflake__create_dynamic_table_info_schema_sql(dynamic_table, relation, compiled_code) }}
    {%- elif catalog_relation.catalog_type == 'BUILT_IN' -%}
        {{ keboola_snowflake__create_dynamic_table_built_in_sql(dynamic_table, relation, compiled_code) }}
    {%- else -%}
...
{% macro keboola_snowflake__create_dynamic_table_info_schema_sql(dynamic_table, relation, sql) -%}
{#-
...
{% macro keboola_snowflake__create_dynamic_table_built_in_sql(dynamic_table, relation, sql) -%}
{#-

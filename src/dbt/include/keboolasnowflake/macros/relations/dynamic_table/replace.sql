{% macro keboola_snowflake__get_replace_dynamic_table_sql(relation, sql) -%}
{#-
...
-#}
    ...
    {%- if catalog_relation.catalog_type == 'INFO_SCHEMA' -%}
        {{ keboola_snowflake__replace_dynamic_table_info_schema_sql(dynamic_table, relation, sql) }}
    {%- elif catalog_relation.catalog_type == 'BUILT_IN' -%}
        {{ keboola_snowflake__replace_dynamic_table_built_in_sql(dynamic_table, relation, sql) }}
    {%- else -%}
...
{% macro keboola_snowflake__replace_dynamic_table_info_schema_sql(dynamic_table, relation, sql) -%}
{#-
...
{% macro keboola_snowflake__replace_dynamic_table_built_in_sql(dynamic_table, relation, sql) -%}
{#-

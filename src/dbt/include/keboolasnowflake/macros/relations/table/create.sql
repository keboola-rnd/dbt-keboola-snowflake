{% macro keboola_snowflake__create_table_as(temporary, relation, compiled_code, language='sql') -%}

    {%- set catalog_relation = adapter.build_catalog_relation(config.model) -%}

    {%- if language == 'sql' -%}
        {%- if temporary -%}
            {{ keboola_snowflake__create_table_temporary_sql(relation, compiled_code) }}
        {%- elif catalog_relation.catalog_type == 'INFO_SCHEMA' -%}
            {{ keboola_snowflake__create_table_info_schema_sql(relation, compiled_code) }}
        {%- elif catalog_relation.catalog_type == 'BUILT_IN' -%}
            {{ keboola_snowflake__create_table_built_in_sql(relation, compiled_code) }}
        {%- elif catalog_relation.catalog_type == 'ICEBERG_REST' -%}
            ...
            catalog_relation.catalog_linked_database_type == 'glue' -%}
                {{ keboola_snowflake__create_table_iceberg_rest_with_glue(relation, compiled_code, catalog_relation) }}
            {%- else -%}
                {{ keboola_snowflake__create_table_iceberg_rest_sql(relation, compiled_code) }}
            {%- endif -%}
        {%- else -%}
    ...
    {%- else -%}
        {% do exceptions.raise_compiler_error("keboola_snowflake__create_table_as macro didn't get supported language, it got %s" % language) %}
...
{% macro keboola_snowflake__create_table_temporary_sql(relation, compiled_code) -%}
{#-
...
{% macro keboola_snowflake__create_table_info_schema_sql(relation, compiled_code) -%}
{#-
...
{% macro keboola_snowflake__create_table_built_in_sql(relation, compiled_code) -%}
{#-
...
{% macro keboola_snowflake__create_table_iceberg_rest_sql(relation, compiled_code) -%}
{#-
...
{% macro keboola_snowflake__create_table_iceberg_rest_with_glue(relation, compiled_code, catalog_relation) -%}
{#-
